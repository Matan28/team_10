  ...
  <div class="container table-responsive">
    <table id="cart" class="table table-hover table-sm">
      ...
      <tfoot>
        ...
        <tr>
          <td>
            <button class="btn btn-warning" @click="saveShoppingCartLocal"><i class="fa fa-angle-left"></i> Save and Continue Shopping</button>
          </td>
      ...
      itemList.map((item) => {
        for (let prodIdx = 0; prodIdx < prodList.length; prodIdx++) {
          if (prodList[prodIdx].id == item.id) {
            if (prodList[prodIdx].totalQuantity < item.quantity) {
              message = `Only ${prodList[prodIdx].totalQuantity} ${item.name} available in stock`;
              isValid = false;
              return;
            }
            break;
